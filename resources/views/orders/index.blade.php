            ...
            <div class="p-3 rounded-lg dark:bg-neutral-800">
                ...
                <div class="flex flex-col gap-3 rounded-lg">
                    @foreach ($order->order_items as $item)
                        @php
                            $discounted_price_at_purchase = $item->price_at_purchase - $item->price_at_purchase * $item->discount_at_purchase / 100
                        @endphp
                        <div class="flex justify-between p-5 rounded-lg dark:bg-neutral-700">
                            ...
                            <div class="flex">
                                <div class="flex flex-col items-start justify-between pr-1 border-r-2 lg:pr-3 border-neutral-800/40">
                                    <div>
                                        <div>
                                            <span class="line-through">{{ $item->price_at_purchase }}$</span>
                                            <span class="text-red-500">{{ $item->discount_at_purchase }}%</span>
                                        </div>
                                        <p>{{ $discounted_price_at_purchase }}$</p>
                                    </div>
                                    <p>Qty {{ $item->quantity }}</p>
                                </div>
                                <div class="flex items-center justify-between pl-2 text-xl lg:pl-5 lg:text-3xl text-stone-200">
                                    {{ $discounted_price_at_purchase * $item->quantity }}$
                                </div>
                            </div>
                        </div>
                    @endforeach
